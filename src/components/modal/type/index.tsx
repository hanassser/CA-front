...
const initFormItems: FormItemData[] = [
  {
    itemType: "input",
    itemProps: {
      rules: [{ required: true, message: "Veuillez saisir le nom de l'autorisation" }],
      label: "le nom de l'autorisation",
      name: "name",
    },
    childProps: {
      placeholder: "le nom de l'autorisation",
    },
  },
  {
    itemType: "input",
...
export default function TypeModal({ info, isShow, onCancel, onOk, menuList }: ModalProps) {
  ...
  return (
    <Modal
      maskClosable={false}
      title={info ? "Modifier les autorisations" : "Ajouter les autorisations"}
      visible={isShow}
      okText="confirmer"
      cancelText="annuler"
      onCancel={close}
